...
flowchart TD
    ...
    subgraph Serving & Deployment
        I -->|Registered Models| J[Flask REST API<br/>src.serving]
        J --> K[Waitress WSGI / Docker Container]
    end
...
- **Core & Pipeline**: Python 3.10+, Pandas, NumPy, SciPy
- **Machine Learning**: `scikit-learn` (Ridge, GradientBoosting, HistGradientBoosting, LogisticRegression)
- **MLOps & Tracking**: MLflow (Experiment tracking & Model Registry)
- **API & Serving**: Flask, Waitress (WSGI server)
- **Containerization**: Docker (Multi-stage build), Docker Compose
- **Quality Assurance**: Pytest (17 API & unit tests), Automated Schema & Metric Auditing
...
#### Payload & Response Schema Examples

##### 1. Flight Price Prediction (`POST /predict/flight-price`)
**Request:**
```json
{
  "from": "Sao Paulo (SP)",
...
```json
{
  "status": "success",
  "recommendations": [
    { "hotel_name": "Hotel Copacabana Palace", "score": 0.91 },
    { "hotel_name": "Grand Hyatt Rio", "score": 0.84 },
    { "hotel_name": "Ibis Botafogo", "score": 0.78 }
  ]
}
```
...
- **Flight price is a deterministic rate card** — `(from, to, flightType, agency)` fixes
  the fare exactly. A random train/test split therefore scores R² ≈ 1.0 by *memorising*
  it. Models are selected on a **held-out-routes** split instead, where the ranking
  inverts and Ridge (R² = 0.857) beats gradient boosting (0.699), which cannot
  extrapolate to unseen city pairs.
- **`time` duplicates `distance`** (r = 0.99999) — dropped for collinearity.
- **Demographics don't predict value**; behaviour does. Age, gender and company all
  correlate ~0 with spend and frequency.
- **Gender is near-unpredictable** from behaviour (strongest |r| = 0.06, classes
  balanced ~50/50) — expect a classifier near chance, and report it against a
  baseline rather than tuning toward the generic >75% target.
- **The user × hotel matrix is dense (82.5%), not sparse** — one hotel per city means
  destination determines the choice.
